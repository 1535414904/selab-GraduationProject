...
const handleSameRoomDrag = (newRows, roomIndex, sourceIndex, destinationIndex) => {
  ...
  if (isDraggingCleaningTime) {
    console.log("禁止直接拖曳清潔時間");
    return; // 禁止直接拖曳清潔時間
  }
  
  // 如果目標位置是清潔時間，調整到相鄰的手術位置
  ...
  const itemsToMove = roomData[sourceIndex + 1] ? 
    roomData.splice(sourceIndex, 2) : // 有清潔時間，移除兩個項目
    roomData.splice(sourceIndex, 1);  // 沒有清潔時間，只移除手術項目
